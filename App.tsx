...
export default function App() {
  ...
  const [view, setView] = useState<ViewMode>(() => {
    // Check if user is logged in, if so go to curriculum
  ...
  // Only theory-only if there's no task at all (empty or undefined)
  ...
  // Lesson is complete only if: task is done (if exists) AND guide has been scrolled
  // Memoize taskComplete to prevent unnecessary recalculations
  ...
  // Track if scroll complete callback has been called for current lesson
  const scrollCompleteCalledRef = useRef(false);
  
  // Reset scroll complete tracking when lesson changes
  useEffect(() => {
    scrollCompleteCalledRef.current = false;
  }, [currentLessonIdx]);

  // Memoize the scroll complete callback to prevent unnecessary re-renders
  const handleGuideScrollComplete = useCallback(() => {
    // Only update if not already true AND not already called for this lesson
  ...
  // Show feature guide after login (only once)
  useEffect(() => {
  ...
  // Auto-unlock next level when lesson is complete (without showing modal)
  useEffect(() => {
  ...
  const achievements = {
    'first-commit': currentLessonIdx >= 2, // Completed first commit lesson
    'branch-master': currentLessonIdx >= 6, // Completed branching lessons
    'merge-wizard': currentLessonIdx >= 8, // Completed merge lessons
    'remote-expert': currentLessonIdx >= 12, // Completed remote lessons
    'all-complete': currentLessonIdx >= LESSONS.length - 1, // Completed all lessons
  };
  ...
  const handleCommand = useCallback(async (cmd: string) => {
    ...
    if (cmd.trim() && cmd !== 'clear') {
      setCommandHistory(prev => [...prev, cmd].slice(-100)); // Keep last 100 commands
    }
        ...
        <header className="h-16 md:h-20 flex items-center justify-between px-2 sm:px-3 md:px-6 lg:px-10 shrink-0 sticky top-2 z-50 bg-white/80 dark:bg-[#0f172a]/80 backdrop-blur-md border border-slate-200 dark:border-white/10 rounded-xl md:rounded-2xl mx-2 md:mx-4 relative min-w-0 overflow-visible">
            ...
            <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 lg:gap-4 min-w-0 flex-shrink">
                {currentLessonIdx !== -1 ? (
                 ...
                 {/* Theme Button - Hidden on mobile, shown in dropdown */}
                 <button 
                    onClick={toggleTheme}
                 ...
                 {/* Map Button - Hidden on mobile, shown in dropdown */}
                 <button 
                     ...
                     <div className="p-4 space-y-2 max-h-[80vh] overflow-y-auto">
                       ...
                       <div className="pt-2 border-t border-slate-200 dark:border-slate-700 space-y-2">
                         ...
                         <button
                           ...
                           className="w-full flex items-center gap-3 px-4 py-3 rounded-lg bg-slate-100 dark:bg-slate-900/80 hover:bg-slate-200 dark:hover:bg-slate-800 transition-all text-left"
                         >
                           <History size={18} className="flex-shrink-0 text-indigo-600 dark:text-indigo-400" />
                           <span className="font-semibold text-slate-900 dark:text-white">Command History</span>
                         </button>
                         ...
                         >
                           ...
                           <span className="font-semibold text-slate-900 dark:text-white">Export History</span>
                         </button>
